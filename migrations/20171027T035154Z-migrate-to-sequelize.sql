...
SELECT id, ip, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
FROM visitors_old;
...
SELECT id, message, hidden, strftime('%Y-%m-%dT%H:%M:%f +00:00', timestamp), strftime('%Y-%m-%dT%H:%M:%f +00:00', timestamp), visitor_id
FROM messages_old;
...
SELECT id, path, label, href, strftime('%Y-%m-%dT%H:%M:%f +00:00', timestamp), strftime('%Y-%m-%dT%H:%M:%f +00:00', timestamp), visitor_id
...
SELECT short_url, url, expiry, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
FROM urls_old;
